...
const StrangerThingsCharacters = memo(() => {
  ...
  return (
    <>
      {/* Vecna - Right side */}
      <motion.div
        className="fixed right-0 top-1/4 z-20 pointer-events-none will-change-[transform,filter]"
        style={{ y: vecnaY, opacity: vecnaOpacity }}
      >
        <motion.img
          src={vecna}
          alt=""
          className="w-[300px] md:w-[400px] lg:w-[500px] h-auto object-contain opacity-40 backface-hidden"
          style={{
          ...
          animate={{
            filter: [
              ...
              'brightness(0.7) saturate(1.8) hue-rotate(0deg)',
              'brightness(0.5) saturate(1.5) hue-rotate(-10deg)',
            ],
          }}
          transition={{
      ...
      <motion.div
        className="fixed left-0 bottom-0 z-20 pointer-events-none will-change-[transform,filter]"
        style={{ y: demogorgonY, opacity: demogorgonOpacity }}
      >
        <motion.img
          src={demogorgon}
          alt=""
          className="w-[250px] md:w-[350px] lg:w-[400px] h-auto object-contain opacity-30 backface-hidden"
          style={{
      ...
      <motion.div
        className="fixed top-0 left-1/2 -translate-x-1/2 z-10 pointer-events-none w-full will-change-[opacity]"
        initial={{ opacity: 0 }}
